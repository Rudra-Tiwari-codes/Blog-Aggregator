...
describe('POST /api/search', () => {
  ...
  it('should handle search with no results', async () => {
    ...
    const response = await request(app).post('/api/search').send({ query: 'Python' }).expect(200);
  ...
  it('should respect query length limits', async () => {
    ...
    const response = await request(app).post('/api/search').send({ query: longQuery }).expect(400);
